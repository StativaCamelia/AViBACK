...
document.addEventListener("DOMContentLoaded", function changeElementsNews() {
  if (value == 4) value = 0;
  for (let i = 1; i <= 6; i++) {
    element = document.getElementById("newss" + i);
    element.innerHTML =
      x[i + value].firstChild.nodeValue.substring(0, 35) +
      "..." +
      '<a class ="read_more" href="index.html"><small><i>Read More</i></small</a>';
  }
  setTimeout(function () {
    for (let i = 1; i <= 6; i++) {
      element = document.getElementById("newss" + i);
      element.innerHTML =
        x[value + i].firstChild.nodeValue.substring(0, 35) +
        "..." +
        '<a class ="read_more" href="index.html"><small><i>Read More</i></small></a>';
    }
    value += 1;
    if (value < 10) changeElementsNews();
  }, 10000);
});
